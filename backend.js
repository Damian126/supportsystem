...
let projectName = 'Support System';
...
async function init(app, con) {
    ...
    setTimeout(async () => {
        ...
        console.log('Designed by https://github.com/itz-hyperz');
    }, 3000);
    licenseCheck("1691773642681V5YS2Q1W5iAus4", config.licenseKey);
...
async function checkAuth(req, res, next) {
    if(req.isAuthenticated()){
        dbcon.query(`SELECT * FROM users WHERE id="${req.user.id}"`, function(err, row) {
            if(err) throw err;
            if(!row[0]) {
                dbcon.query(`INSERT INTO users (id, username, email, password) VALUES ("${req.user.id}", "${req.user.username}", "${req.user.email}", "discord")`, function(err, row) {
                    if(err) throw err;
...
async function licenseCheck(productId, licenseKey) {
    let request = await axios({
        method: 'get',
        url: 'https://store.hyperz.net/api/v1/license/check/' + licenseKey,
        headers: {
    ...
    if(request.data.authorized) {
        return console.log(`${chalk.magenta('[License System]')} License Authorized!`);
    } else {
        console.log(`${chalk.magenta('[License System]')} License Failed!`);
